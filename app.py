...
def generate_data(N, mu, beta0, beta1, sigma2, S):
    ...
    Y =beta0 + beta1 * X+ np.random.normal(loc=mu, scale=np.sqrt(sigma2), size=(N, 1))   # Replace with code to generate Y
    ...
    for _ in range(S):
        ...
        Y_sim = beta0 + beta1 * X_sim + np.random.normal(loc=mu, scale=np.sqrt(sigma2), size=(N, 1))   # Replace with code to generate simulated Y values
    ...
    slope_more_extreme = sum(np.abs(sim_slope) >= np.abs(slope) for sim_slope in slopes) / S  # Replace with code to calculate proportion of slopes more extreme than observed
    intercept_extreme = sum(np.abs(sim_intercept) >= np.abs(intercept) for sim_intercept in intercepts) / S  # Replace with code to calculate proportion of intercepts more extreme than observed
...
def hypothesis_test():
    ...
    if test_type == "greater":
        ...
        p_value = sum(np.abs(simulated_stats - hypothesized_value) >= np.abs(observed_stat - hypothesized_value)) / S

    # TODO 11: If p_value is very small (e.g., <= 0.0001), set fun_message to a fun message
    fun_message = "Wow! You've encountered a very rare event!" if p_value <= 0.0001 else None
...
def confidence_interval():
    ...
    if confidence_level > 1:
        confidence_level /= 100  # to convert percentage to decimal 
